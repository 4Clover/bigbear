...
- The hourly expire-holds cron only cancels `PENDING` holds with `paymentClaimedAt = null`. A claimed-but-unverified Hold (guest pressed "I've sent the payment") is **not** auto-cancelled — the money may genuinely be in flight, so the owner resolves it manually (approve or cancel from the bookings table).
